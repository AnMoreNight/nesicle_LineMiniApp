  ...
  return (
    <div className="min-h-dvh px-6 py-10">
      <div className="mb-6 text-center">
        <p className="text-2xl font-extrabold">ネシクル</p>
      </div>

      <div className="rounded-lg bg-primary-soft px-4 py-3 text-lg font-bold text-primary">
        {data.referrerDisplayName} さんから紹介が届いています
      </div>

      <p className="mb-3 mt-6 text-lg font-extrabold text-ink-muted">紹介されたサービス</p>
      <div className="space-y-3">
        {data.cases.map((c) => (
          <div key={c.id} className={card}>
            ...
            <p className="mt-1 text-base leading-relaxed text-ink-muted">{c.summary}</p>
          </div>
        ))}
      </div>

      <p className="mt-6 rounded-md bg-surface-muted px-3 py-2.5 text-base leading-relaxed text-ink-muted">
