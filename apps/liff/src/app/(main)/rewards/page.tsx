...
function Section({ title, items }: { title: string; items: ReferrerApplicationDto[] }) {
  ...
  return (
    <section>
      <p className="mb-3 text-lg font-extrabold text-ink-muted">{title}</p>
      <div className="space-y-2">
        {items.map((a) => (
          <div key={a.id} className={clsx(card, "flex items-center justify-between")}>
            <div>
              <p className="text-lg font-bold">{a.caseTitle}</p>
              <p className="text-base text-ink-muted">{a.applicantName} さん</p>
            </div>
            {a.rewardAmount != null ? <MoneyText amount={a.rewardAmount} className="text-xl" /> : <span className="text-base text-ink-muted">査定中</span>}
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <div>
      ...
      <main className="space-y-5 px-5 py-4">
        <section className="rounded-lg bg-ink p-4 text-white shadow-float">
          <p className="text-base font-bold text-white/70">累計報酬(確定+支払済み)</p>
          <p className="mt-1 text-5xl font-extrabold tabular-nums">{formatYen(rewards?.lifetimeTotal ?? 0)}</p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-center text-base font-bold">
            <div className="rounded-md bg-white/10 py-2.5">
        ...
        {!rewards?.hasBankAccount && (
          <div className={clsx(card, "flex items-center justify-between gap-3 border-warning/40 bg-warning-soft")}>
            <p className="text-base font-bold text-warning">報酬の受け取りには振込先口座の登録が必要です</p>
            <Link href="/mypage/bank" className={clsx(btnPrimary, "!px-3 !py-2 text-base")}>
              登録する
            </Link>
          </div>
        )}
        ...
        {confirmed.length + paid.length + pending.length === 0 && (
          <p className="text-center text-base text-ink-muted">まだ報酬の実績がありません。</p>
        )}
      </main>
    </div>
  );
