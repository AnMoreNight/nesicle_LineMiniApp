    ...
    <div>
      ...
      <main className="space-y-6 px-5 py-5">
        <section className="rounded-lg bg-gradient-to-br from-primary to-[#048848] p-5 text-white shadow-float">
          ...
          <p className="mt-1 text-4xl font-extrabold tabular-nums">{formatYen((rewards?.confirmedTotal ?? 0) + (rewards?.paidTotal ?? 0))}</p>
          ...
          {!rewards?.hasBankAccount && (
            <Link href="/mypage/bank" className="mt-3 block text-sm font-bold underline underline-offset-2 opacity-90">
              振込先口座を登録する →
            </Link>
          )}
        </section>
        ...
        <section>
          <div className="mb-3 flex items-end justify-between">
            <p className="text-lg font-extrabold">おすすめ案件</p>
            <Link href="/cases" className="text-sm font-bold text-primary">
              すべて見る
            </Link>
          </div>
