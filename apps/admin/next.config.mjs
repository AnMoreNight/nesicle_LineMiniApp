...
const nextConfig = {
  ...
  // Type errors are already caught by `npm run typecheck` (plain `tsc --noEmit`, which passes
  // cleanly). Next's own build-time type-checking spawns a separate fork-ts-checker worker
  // process that has been observed to deadlock in this environment; skip it here since it's
  // redundant with the standalone typecheck.
